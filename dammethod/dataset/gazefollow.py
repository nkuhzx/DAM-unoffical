...
class GazeFollowDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.imshow:
            ...
            eyes_outpix=[eye_x*self.input_size,eye_y*self.input_size]
            eyes_pixel=[org_eyex*orgimg_show.size[0],org_eyey*orgimg_show.size[1]]

            if self.test:
                gaze_outpix=[gaze_mean[0]*self.input_size,gaze_mean[1]*self.input_size]
                gaze_pixel=[gaze_mean[0]*orgimg_show.size[0],gaze_mean[1]*orgimg_show.size[1]]
            else:
                gaze_outpix = [gaze_x * self.input_size, gaze_y * self.input_size]
                gaze_pixel=[org_gazex*orgimg_show.size[0],org_gazey*orgimg_show.size[1]]
            ...
            # print(gv_field.shape,gaze_vector.shape)
            ...
            ax[1][0].imshow(orgimg_show)

            ax[1][0].scatter(eyes_pixel[0],eyes_pixel[1])
            ax[1][0].scatter(gaze_pixel[0],gaze_pixel[1])

            # ax[1][1].scatter(eyes_pixel[0],eyes_pixel[1])
            # ax[1][1].scatter(gaze_pixel[0],gaze_pixel[1])
