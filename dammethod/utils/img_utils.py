...
def generate_gaze_field(eye_point,size=(224,224)):
    ...
    eye_pix=np.array(eye_point)*np.array([width,height])

    grid-=eye_pix.reshape([2,1,1]).astype(np.float32)

    grid=to_torch(grid)

    grid=F.normalize(grid,p=2,dim=0)
